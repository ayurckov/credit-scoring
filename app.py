...
st.write("Новая кредитная карта с мгновенным оформлением")

with st.form("Подавть заявку"):
    age = st.number_input("Ваш возраст", min_value=18)
    income = st.number_input("Ваш доход (в тысячах руб)", min_value=0)
...
if submit:
    ...
    if response.json()["approved"]:
        ...
        st.success("К сожалению ваша заявка отклонена, но мы подобрали для вас альтернативу!")
